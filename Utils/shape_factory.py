...
def shape_base_drain():
    """
    Creates a basic drain shape using a circular wire as the base.

    Returns:
        TopoDS_Shape: The resulting drain shape.
    """
    wire = wire_circle()  # Create a circular wire

    # Scale the wire to the desired size
    trsf = gp_Trsf()
    trsf.SetScale(DRAIN_RCS.Location(), DRAIN_R)
    bresp_trsf = BRepBuilderAPI_Transform(wire, trsf)
    wire = topods.Wire(bresp_trsf.Shape())

    # Create a face from the wire and extrude it to create a prism
    base_face = BRepBuilderAPI_MakeFace(wire).Face()
    shape = BRepPrimAPI_MakePrism(base_face, gp_Vec(0, 0, DRAIN_T)).Shape()
