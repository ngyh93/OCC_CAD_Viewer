...
def ray_segment_set_intersect(ray_pnt, ray_dir, segs):
    """
    Compute intersections between a ray and multiple segments using parallel processing
    and pre-allocation optimization.
    
    Args:
        ray_pnt: Origin point of the ray
        ray_dir: Direction of the ray
        segs: Array of segments, each containing two points
        
    Returns:
        Array of valid intersection distances
    """
    n = segs.shape[0]
    # Pre-allocate array for all results
    temp_intersects = np.full(n, -np.inf, dtype=np.float64)
    
    # Process segments in parallel
    for i in nb.prange(n):
        val = ray_segment_intersect(ray_pnt, ray_dir, segs[i][0], segs[i][1])
        temp_intersects[i] = val
    
    # Filter valid intersections after parallel computation
    return temp_intersects[temp_intersects >= 0.0]
